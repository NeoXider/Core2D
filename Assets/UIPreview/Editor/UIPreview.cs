    ...
    public class UIPreview : ObjectPreview
    {
        ...
        */
        ...
        public static Texture2D CreateAssetPreview(Object target)
        {
            ...
            {
                Debug.LogWarning("UIPreview: Preview of Prefabs with ParticleSystems is not supported due to a Unity Bug/Limitation (Issue#:1399450 and similar).");
                return ErrorTexture;
            }
            ...
            // If root rect transform width/height is zero and anchors are not equal the equalize them
            // since GetWorldCorners() apparently does return all zero results for those.
            // INVESTIGATE: Find a ways to force update the rect transform so it is sized correctly (have not yet found a way).
            var rt = obj.GetComponent<RectTransform>();
            if (rt.rect.width == 0 || rt.rect.height == 0)
            {
                // Fallback width/height is reference resolution.
                float canvasWidth = settings.ReferenceResolution.x;
                float canvasHeight = settings.ReferenceResolution.y;
                // If there is a rect transform on the canvas then take the resolution from there.
                ...
                else if (canvas.TryGetComponent<RectTransform>(out var canvasTransform))
                {
                    canvasWidth = canvasTransform.rect.width;
                    canvasHeight = canvasTransform.rect.height;
                }

                var minAnchor = rt.anchorMin;
                var maxAnchor = rt.anchorMax;
                var anchorDelta = maxAnchor - minAnchor;

                // Calc root element size based on anchors
                var elementWidth = canvasWidth * Mathf.Abs(anchorDelta.x);
                var elementHeight = canvasHeight * Mathf.Abs(anchorDelta.y);
                // Scale up if too small.
                if(elementWidth < 10 && elementHeight < 10)
                {
                    float factor = Mathf.Min(100, 100f / Mathf.Min(elementWidth, elementHeight));
                    elementWidth *= factor;
                    elementHeight *= factor;
                }

                // Collapse anchors to center
                var centerAnchor = minAnchor + anchorDelta * 0.5f;
                rt.anchorMin = centerAnchor;
                rt.anchorMax = centerAnchor;

                // Update size (since it is 0/0 and not updated automatically).
                rt.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, elementWidth);
                rt.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, elementHeight);

                // Force another layout update since we have changed the size.
                for (int i = rectTransforms.Length - 1; i >= 0; i--)
                {
                    LayoutRebuilder.ForceRebuildLayoutImmediate(rectTransforms[i]);
                }
            ...
            // WIP / Not working.
            // var particleSystems = checkForParticlesGO.GetComponentsInChildren<ParticleSystem>();
            // particleSystems[0].Simulate(0.3f, withChildren: true, restart: true);
            ...
            if (renderTexture == null || renderTexture.width != texWidth || renderTexture.height != texHeight)
            {
                ...
                // To avoid this error: https://issuetracker.unity3d.com/issues/urp-rendergraph-warning-thrown-when-rendertexture-has-depth-stencil-format-set-to-none
                depth = 16;
#endif
                renderTexture = RenderTexturePool.Get(texWidth, texHeight, depth: depth, RenderTextureFormat.Default);
            }
