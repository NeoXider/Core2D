...
namespace InfinityCode.UltimateEditorEnhancer
{
    public static class GameObjectUtils
    {
        ...
        public static IEnumerable<GameObject> GetSiblings(GameObject gameObject)
        {
            ...
            if (parent)
            {
                for (int i = 0; i < parent.childCount; i++)
                {
                    ...
                    if (t != transform) yield return t.gameObject;
                }
            }
            else
            {
                GameObject[] gameObjects = transform.gameObject.scene.GetRootGameObjects();
                for (int i = 0; i < gameObjects.Length; i++)
                {
                    GameObject go = gameObjects[i];
                    if (go != transform.gameObject) yield return go;
                }
            }
        }
