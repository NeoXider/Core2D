...
namespace InfinityCode.UltimateEditorEnhancer.HierarchyTools
{
    ...
    public static class BestIconDrawer
    {
        ...
        private static Texture prefabIcon => EditorIconContents.prefab.image;
        private static Texture prefabVariantIcon => EditorIconContents.prefabVariant.image;
        private static Texture unityLogoTexture => EditorIconContents.unityLogo.image;

        static BestIconDrawer()
        {
            hierarchyWindows = new HashSet<int>();
            HierarchyItemDrawer.Register(nameof(BestIconDrawer), DrawItem, HierarchyToolOrder.BestIcon);
        ...
        public static Texture GetGameObjectIcon(GameObject go)
        {
            ...
            if (!texture) return EditorIconContents.gameObject.image;
            return texture;
        }
        ...
        private static bool GetTexture(HierarchyItem item, out Texture texture)
        {
            ...
            else if (!item.target) texture = unityLogoTexture;
            else return false;
