...
namespace InfinityCode.UltimateEditorEnhancer.Windows
{
    ...
    public partial class Bookmarks : EditorWindow
    {
        ...
        private static bool sortedItemsDirty;
        private static ProjectBookmark[] sortedProjectItems;
        private static Dictionary<SceneReferences, List<SceneBookmark>> sortedSceneItems;

        private string _filter = "";
        private string activeLabel;
        private bool focusOnSearch;
        private BookmarkSortType sortType = BookmarkSortType.Default;
        ...
        public static JsonArray json
        {
            get
            {
                ...
                foreach (ProjectBookmark item in BookmarkReferences.items)
                {
                    JsonObject obj = new JsonObject();
                    obj.Add("path", AssetDatabase.GetAssetPath(item.target), JsonValue.ValueType.STRING);
                    jArr.Add(obj);
                }
            ...
            set
            {
                ...
                foreach (JsonItem v in value)
                {
                    ...
                    if (BookmarkReferences.Any(i => i.target == obj)) continue;

                    ProjectBookmark item = new ProjectBookmark(obj);
                    BookmarkReferences.Add(item);
                }
        ...
        public static void Add(Object target) 
        {
            ...
            {
                if (BookmarkReferences.Any(i => i.target == target)) return;
        ...
        public static bool Contains(Object target)
        {
            ...
            return BookmarkReferences.Any(i => i.target == target);
        }
        ...
        {
            if (BookmarkReferences.count == 0) return;
        ...
        private void DrawProjectItems()
        {
            if (sortedProjectItems == null) UpdateSorting();
            if (sortedProjectItems.Length == 0) return;
            
            showProjectItems = GUILayout.Toggle(showProjectItems, "Project Items", EditorStyles.foldoutHeader);
            if (!showProjectItems) return;
                    
            if (gridSize > MinGridSize) DrawGridItems(sortedProjectItems);
            else DrawTreeItems(ref projectItemsList, sortedProjectItems);
        }

        private void DrawSceneItems()
        {
            if (sortedSceneItems == null) UpdateSorting();
            if (sortedSceneItems.Count == 0) return;
            
            showSceneItems = GUILayout.Toggle(showSceneItems, "Scene Items", EditorStyles.foldoutHeader);
            if (!showSceneItems) return;
            
            if (sceneItemsList == null) sceneItemsList = new Dictionary<SceneReferences, ReorderableList>();
            bool multiScene = sortedSceneItems.Count > 1;
            
            foreach (var pair in sortedSceneItems)
            {
        ...
        {
            GUIContent content = TempContent.Get(EditorIconContents.alphabeticalSorting.image, "Sort Bookmarks");
            if (!GUILayoutUtils.ToolbarButton(content)) return;
            
            GenericMenuEx menu = GenericMenuEx.Start();
            menu.Add("Default", sortType == BookmarkSortType.Default, () => SetSortType(BookmarkSortType.Default) );
            menu.Add("Sort by Name", sortType == BookmarkSortType.Name, () => SetSortType(BookmarkSortType.Name));
            menu.Add("Sort by Type", sortType == BookmarkSortType.Type, () => SetSortType(BookmarkSortType.Type));
            menu.Show();
        }
        ...
        private static string GetItemTooltip(BookmarkItem item)
        {
            StringBuilder builder = StaticStringBuilder.Start();

            builder.Append(item.tooltip)
                .Append("\n(")
                .Append(item.target.GetType().Name)
                .Append(")\n\n")
                .Append("Click - Select Object");
            
            if (item.target is DefaultAsset) builder.Append("\nDouble Click - Open Folder");
        ...
        private static void InitFolderItems(BookmarkItem folderItem)
        {
            ...
            tempItems.Sort((item1, item2) => EditorUtility.NaturalCompare(item1.title, item2.title));
        ...
        {
            if (Contains(target)) menu.Add("Remove Bookmark", () => Remove(target));
        ...
        private void OnGUI()
        {
            ...
            if (sortedItemsDirty) UpdateSorting();
        ...
        private void ProcessEvents()
        {
            ...
            if (!position.Contains(e.mousePosition + position.position)) return;
        ...
        public static void Remove(Object item)
        {
            ...
            {
                BookmarkReferences.RemoveAll(i => i.target == item);
                cachedLabels = null;
                _folders = null;
                Save();
            }
        ...
        private static void Remove(BookmarkItem item)
        {
            ...
            if (item.isProjectItem)
            {
                BookmarkReferences.Remove(item as ProjectBookmark);
                return;
            }
        ...
        private void SetSortType(BookmarkSortType type)
        {
            sortType = type;
            UpdateSorting();
        }
        ...
        private void UpdateFilteredItems()
        {
            ...
            if (selectedFolderItems == null)
            {
                query = BookmarkReferences.Select(i => i as BookmarkItem);
        ...
        {
            if (sortType == BookmarkSortType.Default)
            {
                sortedProjectItems = BookmarkReferences.items;
                sortedSceneItems = sceneItems;
                return;
            } 
            
            List<ProjectBookmark> projectItems = new List<ProjectBookmark>(BookmarkReferences.items);
            sortedSceneItems = new Dictionary<SceneReferences, List<SceneBookmark>>();
            
            if (sortType == BookmarkSortType.Name)
            {
                projectItems.Sort(new ProjectBookmarkComparer());
                
                foreach (var pair in sceneItems)
                {
                    sortedSceneItems.Add(pair.Key, pair.Value.OrderBy(i => i.title).ToList());
                }
            }
            else if (sortType == BookmarkSortType.Type)
            {
                projectItems.Sort(new ProjectBookmarkTypeComparer());
                
                foreach (var pair in sceneItems)
                {
                    sortedSceneItems.Add(pair.Key, pair.Value.OrderBy(i =>
                    {
                        if (!i.target) return "";
                        return i.target.GetType().Name;
                    }).ThenBy(i => i.title).ToList());
                }
            }
            sortedProjectItems = projectItems.ToArray();
        }
    }
}