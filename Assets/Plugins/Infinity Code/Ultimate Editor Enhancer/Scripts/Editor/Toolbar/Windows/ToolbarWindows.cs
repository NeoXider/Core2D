    ...
    public static partial class ToolbarWindows
    {
        ...
        private static void FocusWindow(object userdata)
        {
            ...
            foreach (EditorWindow window in activeWindows)
            {
                ...
                if (type == typeof(SceneView) || type == GameViewRef.type) continue;
                EditorWindow newWindow = EditorWindow.GetWindow(type, false, record.title);

                if (type != SceneHierarchyWindowRef.type) return;
            ...
            foreach (EditorWindow window in activeWindows)
            {
                if (!window || window.GetType() != type) continue;
        ...
        private static bool IgnoreWindow(EditorWindow window)
        {
            ...
            if (typeName == "ContextMenu") return true;
#endif
#if UNITY_6000_3_OR_NEWER
            if (type.Name == "MainToolbarWindow") return true;
#endif
            return false;
        }
...
#endif
            if (Prefs.windowsToolbarIcon) EditorApplication.update += CacheWindows;
        }
...
#if UNITY_6000_3_OR_NEWER
        private const string ToolbarPath = "Ultimate Editor Enhancer/Windows";
        ...
        {
            buttonContent = new GUIContent(Icons.windows, "Windows");
            return new MainToolbarDropdown(buttonContent, _ => ShowMenu());
        }
